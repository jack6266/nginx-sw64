...
#define ngx_file_fs_size(sb)                                                 \
    (((sb)->st_blocks * 512 > (long unsigned int) (sb)->st_size  && (sb)->st_blocks * 512 < (long unsigned int) (sb)->st_size + 8 * (sb)->st_blksize)        \
     ? (sb)->st_blocks * 512 : (long unsigned int) (sb)->st_size)
...
#define ngx_de_fs_size(dir)                                                  \
    ngx_max((long unsigned int) (dir)->info.st_size, (dir)->info.st_blocks * 512)
